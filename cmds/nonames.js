...
module.exports = {
  ...
  async execute(context) {
    ...
    if (!hasPermission) {
      const senderRole = await getUserRole(context.peerId, context.senderId);
      const senderRoleName = await getRoleName(context.peerId, senderRole);
      return context.reply(`⛔ Доступ запрещён | Для использования команды /nonames требуется приоритет 20 или выше\n👤 Ваша роль: ${senderRoleName} (приоритет ${senderRole})`);
    }
